...
class KspTypeElementTest {
    @Test
    fun qualifiedNames() {
        val src1 = Source.kotlin("Foo.kt", """
            class TopLevel
        """.trimIndent())
        val src2 = Source.kotlin("Bar.kt", """
            package foo.bar
            class InFooBar
        """.trimIndent())
        runKspTest(
    ...
    fun nestedClassName() {
        ...
        runKspTest(sources = listOf(src), succeed = true) { invocation ->
            invocation.processingEnv.requireTypeElement("foo.bar.Outer").let {
                ...
                assertThat(it.enclosingElement).isNull()
            }
            invocation.processingEnv.requireTypeElement("foo.bar.Outer.Inner").let {
                ...
                assertThat(it.enclosingElement).isEqualTo(
    ...
    fun modifiers() {
        ...
        runKspTest(sources = listOf(src), succeed = true) { invocation ->
            ...
            fun getModifiers(qName : String):Set<String> = getModifiers(invocation.processingEnv
